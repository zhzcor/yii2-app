...
(function(window, $) {
        ...
        init: function (params) {
            ...
            if (this.options.bEvent) {
                $(this.options.searchForm + ' input').on(this.options.searchInputEvent, function () {
                	if(!$("#me-table-buttons").hasClass('pull-right')){
                		self.table.draw();
                	}
                });
    ...
    meTables.extend({
        ...
        htmlCreate: function(params) {
            return '<div id="formdata-'+params.name+'" data-id="'+params.key+'" data-type="'+params.control+'"></div>';
        },
                ...
                if (data !== undefined) {
                    for (var i in data) {
                        ...
                        if (objForm[i] !== undefined && objForm[i].type !== "password") {
                            ...
                            objForm[i].value = tmp;
                        }else if($('#formdata-'+i).length >0){   //图片展示
                        	var key = $('#formdata-'+i).attr('data-id');
                        	var control = $('#formdata-'+i).attr('data-type');
                        	var domain = window.location.host;
                        	if($('#formdata-'+i).attr('data-id') != undefined && key != undefined){
                        		try {  
                        			var dataJson = $.parseJSON(data[i]);
                        			if(control == 'image'){
                        				var htmls = '<a class="pre-image" href="http://'+domain+'/'+dataJson['orig']+'" data-lightbox="form-image-preview"><img src="http://'+domain+'/'+dataJson['small']+'"/></a>';
                        				$('#formdata-'+i).html(htmls);
                        			}else{
                        				for ( var c in dataJson) {
    										if(c == key){
    											$('#formdata-'+i).html(dataJson[c]);
    										}
    									}
                        			}
                        	    } catch (e) {  
                        	    	$('#formdata-'+i).html(data[i]);
                        	    }  
                        	}else{
                        		$('#formdata-'+i).html(data[i]);
                        	}
        ...
        // 显示标签
        valuesLabels: function(data, datas) {
            return datas[data];
        }
